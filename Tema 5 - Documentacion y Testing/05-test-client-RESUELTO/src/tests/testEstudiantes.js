...
async function testGetWithInvalidIdentifier() {
  ...
  try {
    ...
    console.log(
      "get by inexisting id: error - se encontró un estudiante inexistente"
    );
  } catch (err) {
    if (err.statusCode == 404) {
...
async function testGetByRangoEdad() {
  ...
  try {
    ...
    const desde = 15;
    const hasta = 35;
    const estudiantes = await cli.buscarPorParametros({
      edadMin: desde,
      edadMax: hasta
    });
    validarEstudiantes(estudiantes);
...
async function testGetByDni() {
  ...
  try {
    ...
    if (!estudiante) {
      console.log(
        "get by existing dni: error - no se encontró un estudiante existente"
      );
    } else {
      validarEstudiante(estudiante);
...
async function testDeleteWithInvalidIdentifier() {
  ...
  try {
    ...
    await cli.borrarEstudiante(id);
    console.log(
      "delete by inexistent id: error - se borró algo que no debería existir"
    );
  } catch (err) {
    if (err.statusCode == 404) {
...
async function testPutWithValidIdentifier() {
  ...
  try {
    ...
    if (buscado.edad == nuevaEdad) {
      ...
      console.log(
        "put by existing id: error - se encontró el mismo estudiante sin reemplazar"
      );
    }
  } catch (err) {
    console.log(err.message);
...
async function testPutWithInvalidIdentifier() {
  ...
  try {
    ...
    console.log(
      "put by inexistent id: error - se reemplazó un estudiante inexistente"
    );
  } catch (err) {
    if (err.statusCode == 404) {
